...
export async function updateLinkDestinations(
  ...
  const destinationsParsed = destinationsSchema.safeParse(destinations);

  const db = getDb();

  if (!destinationsParsed.success) {
    console.log(destinationsParsed.error);
    return;
  }

  await db
    .update(links)
    .set({ destinations: JSON.stringify(destinationsParsed.data) })
    .where(eq(links.linkId, linkId));

  return destinationsParsed.data;
}
